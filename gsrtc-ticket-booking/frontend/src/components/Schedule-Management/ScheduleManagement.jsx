...
const ScheduleManagement = () => {
  const { routes, buses, schedules, setSchedules, admin } = useContext(AppContext);
  ...
  const fetchSchedules = async () => {
    const res = await axios.get(`/api/busTrip/trips/${admin.depotId}`);
  ...
  const handleAddOrUpdate = async () => {
    ...
    try {
      if (editingId) {
        ...
        await axios.post("/api/busTrip/add", scheduleData);
