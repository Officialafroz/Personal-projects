...
const StopManagement = ({ route, updateRoute }) => {
  ...
  useEffect(() => {
    ...
    axios.get(`/api/busRouteStops/stops/${route.routeId}`)
  ...
  const addStop = async (stop) => {
    try {
      const res = await axios.post(`/api/busRouteStops/addStop`, stop);
  ...
  const deleteStop = async (stopName) => {
    ...
    try {
      await axios.delete(`/api/busRouteStops/delete/${stopName}`);
