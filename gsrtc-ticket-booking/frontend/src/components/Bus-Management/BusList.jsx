...
const BusList = () => {
  const { buses, setBuses, admin } = useContext(AppContext);
  ...
  const fetchBuses = async () => {
    try {
      const response = await axios.get(`/api/bus/busList/${admin.depotId}`);
  ...
  const handleRemove = async (busNumber) => {
    ...
    try {
      await axios.delete(`/api/bus/deleteBus/${busNumber}`);
